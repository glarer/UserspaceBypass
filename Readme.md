...
6. Start a small test here to check the setting. Codes lie in `source_codes/io_file`. We have modified the `syscall_read` codes to have 11 times read function tests. The first time read test for the boost period, and the 10 times followed for evaluation.
   1. Firstly, make a big file in toRead folder named "test.file". We use `dd` to build a 16Gbytes file, e.g., `dd if=/dev/zero of=test.file bs=1M count=16384`
   2. Check codes in `io_file/syscall_read.c`: 
        * FILE_POS -> 1
   3. `make` in `io_file/`.
   4. Run `./syscall_read 1024` for 1024 bytes every read. Run `strace` to get the syscall `pread64()` address: `sudo strace -ip $(pidof syscall_read)`
   5. Modify `zz_daemon/main.c` and add this syscall address in array `targets[]`. `make` to re-compile the daemon program.
   6. Insert the kernel module in `zz_lkm/` and run the daemon program in `zz_daemon/`.
   7. Re-run the `iofile/syscall_read` program by using `./syscall_read 1024` in `iofile/` folder. The boost period will happen in the first read function of the program(we repeat the read function 11 times.), and the 10 times followed will enjoy the boosting.
   8. Check `dmesg` for boosting log.
   9. Run `sudo rmmod zz_lkm` to uninstall the module and stop the experiment.
